...
void HaloParallelWorld::Construct()
{
    ...
    G4double nDivisions = 250.0;

    G4Tubs *ghostTubsDetector = new G4Tubs("GhostTubsDetector", 0*cm, 10*cm+0.5*cm, nDivisions*0.5*mm, 0.*M_PI*rad, 2.*M_PI*rad);
    G4LogicalVolume *ghostTubsDetectorLog = new G4LogicalVolume(ghostTubsDetector, 0, "GhostTubsDetectorLog");
    new G4PVPlacement(0, G4ThreeVector(0, 0, nDivisions*0.5*mm), ghostTubsDetectorLog, "GhostTubsDetectorPhys", worldLog, 0, 0);

    G4VSolid* ghostRhoTubsDetector = new G4Tubs("GhostRhoTubsDetector", 0*cm, 10*cm+0.5*cm, 0.5*mm, 0.*M_PI*rad, 2.*M_PI*rad);
    ...
    new G4PVParameterised("Cylinders", LogicGhostRhoTubsDetector, ghostTubsDetectorLog, kZAxis, 2750, cylinderParam);
